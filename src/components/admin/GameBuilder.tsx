        ...
        return (
          <div key={g.id} className="panel p-3 space-y-2">
            <div className="flex items-center gap-3">
              ...
              <button
                type="button"
                onClick={() => setOpenSpec(isOpen ? null : g.id)}
            ...
            {isOpen && (
              <div className="space-y-2 pt-2 border-t border-white/10 text-sm">
                <SpecRow label="🎯 מטרה" body={def.aiSpec.purpose} />
                <SpecRow
                  label="📺 מסך הקרנה"
                  body={def.aiSpec.stageDisplay}
                />
                <SpecRow label="📱 טלפון" body={def.aiSpec.playerDisplay} />
                <SpecRow
                  label={`🤖 הנחיות ל-GPT (${def.aiSpec.questionCount} שאלות)`}
                  body={def.aiSpec.instructions}
                  pre
                />
              </div>
            )}
...
function SpecRow({ label, body, pre }: { label: string; body: string; pre?: boolean }) {
  return (
    <div>
      <div className="text-xs text-gold-light font-bold mb-0.5">{label}</div>
      <div className={`text-xs text-muted leading-relaxed ${pre ? 'whitespace-pre-line' : ''}`}>
        {body}
      </div>
    </div>
  );
